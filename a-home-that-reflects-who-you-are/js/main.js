...
var populateMainDropdown = function() {
	
	$("#menu-dropdown .dropdown-menu").html('');
	$("#menu-dropdown .dropdown-menu").html( $("#main-nav > nav  > ul").html() );
};
...
var splashAdjust = function() {
	var scrPos = $(window).scrollTop();
	var splashPos = $("#splashAnchor").offset().top;
	var formThresh = splashPos - scrPos;
	var formWidth = $("#splashDip").outerWidth();
	$("#splashWrapper").css('width', formWidth + 'px');
	
	if (formThresh <= 30) {
		$("#splashWrapper").addClass('fixed');
	} else {
		$("#splashWrapper").removeClass('fixed');
	}
};
...
var paddingAdjust = function() {
	...
	$('.home-slider-sec .home-slider-container .home-slider .item').css('padding-top', headerHeight + 'px');
	$('.home-slider-sec .home-slider-container .home-slider .item').css('padding-bottom', footerHeight + 'px');
};

$('body').on('click', '.dropdown-menu .dropdown .dropdown-toggle', function(e) {
	var elem = $(this);
	//$('#menu-dropdown .dropdown-menu .dropdown.open .dropdown-menu').toggle();
	//$('#menu-dropdown .dropdown-menu .dropdown.open').removeClass('open');
	//$(elem).parent('.dropdown').addClass('open');
	$(elem).next('.dropdown-menu').toggle();
	e.stopPropagation();
	e.preventDefault();
});
